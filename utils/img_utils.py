...
class Imread_Modcrop:
    ...
    def __call__(self, hr_filename, lr_filename, is_train, dtype=np.uint8):
        hr_img = imread(hr_filename)
        lr_img = imread(lr_filename)
        lr_h, lr_w = lr_img.shape[:2]
        # hr_h, hr_w = hr_img.shape[:2]
        if is_train:
            h = int(lr_h / self.scale) * self.scale
            w = int(lr_w / self.scale) * self.scale
            lr_img = lr_img[:h, :w]
            hr_img = hr_img[:h * self.scale, :w * self.scale]
        if lr_h % 2 != 0:
            lr_img = cv2.copyMakeBorder(lr_img, 0, 1, 0, 0, borderType=cv2.BORDER_REFLECT)
            hr_img = cv2.copyMakeBorder(hr_img, 0, 4, 0, 0, borderType=cv2.BORDER_REFLECT)
        if lr_w % 2 != 0:
            lr_img = cv2.copyMakeBorder(lr_img, 0, 0, 0, 1, borderType=cv2.BORDER_REFLECT)
            hr_img = cv2.copyMakeBorder(hr_img, 0, 0, 0, 4, borderType=cv2.BORDER_REFLECT)
